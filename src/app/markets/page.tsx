...
import { useToastContext, ToastProvider } from '@/components/Toast';
...
function MarketsPageContent() {
  ...
  return (
    <div className='min-h-screen w-screen bg-gradient-to-b from-gray-900 to-black overflow-x-hidden'>
      {/* Header */}
      <header className='w-full border-b border-gray-800 bg-gray-900/50 backdrop-blur-sm sticky top-0 z-40'>
        <div className='w-full px-18 md:px-24 lg:px-36 py-4 flex items-center justify-between'>
          <div className='flex items-center gap-3'>
            <div className='w-10 h-10 bg-gradient-to-r from-[#3EB489] to-[#8ED6C1] rounded-xl flex items-center justify-center shadow-lg'>
              <span className='text-white font-bold text-xl'>S</span>
            </div>
            <span className='text-white font-bold text-xl'>SuperRareBets</span>
          </div>
          <div className='flex items-center gap-3'>
            {isLoggedIn && address ? (
              <>
                <div className='text-white/90 text-sm font-medium px-3 py-1.5 bg-gray-800/50 rounded-lg'>
                  {address.slice(0, 6)}...{address.slice(-4)}
                </div>
                <button
                  onClick={handleDisconnect}
                  className='flex items-center justify-center p-2 rounded-lg hover:bg-gray-800/50 transition-colors'
                  title='Disconnect'
                >
                  <Image
                    src={disconnectIcon}
      ...
      {/* Main Content */}
      <main className='w-full px-18 md:px-24 lg:px-36 py-8'>
        ...
        <div className='mb-8 flex flex-col md:flex-row gap-4 items-center justify-center'>
          <div className='relative w-full md:w-auto md:max-w-md'>
            <input
              ...
              className='w-full px-4 py-2 pl-10 bg-gray-800 border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:border-[#3EB489] focus:outline-none'
            />
          ...
          <div className='flex items-center gap-4 w-full md:w-auto md:max-w-md justify-center md:justify-end'>
            <select className='px-4 py-2 bg-gray-800 border border-gray-700 rounded-lg text-white text-sm focus:border-[#3EB489] focus:outline-none'>
            ...
            <div className='flex items-center gap-2'>
              ...
              <Button
                ...
                className='px-4 py-2 bg-[#3EB489] hover:bg-[#3EB489]/90 text-white rounded-lg disabled:opacity-50 disabled:cursor-not-allowed'
              >
        ...
        <div className='mb-8 flex gap-2 overflow-x-auto pb-2 justify-center'>
          {['All', 'New', 'Expiring Soon', 'Sports', 'Politics', 'Entertainment'].map((category) => (
            <button
              key={category}
              onClick={() => {
                if (category === 'All') setActiveTab('bets');
              }}
              className={`px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap ${
                category === 'All' && activeTab === 'bets'
        ...
        {activeTab === 'bets' && (
          <>
            {loading ? (
              <div className='text-center py-12 text-white/70'>Loading markets...</div>
            ) : bets.length === 0 ? (
              <div className='text-center py-12 text-white/70'>No markets available</div>
            ) : (
    ...
    <div className='fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4'>
      <div className='bg-gray-900 rounded-xl p-6 max-w-md w-full max-h-[90vh] overflow-y-auto'>
        ...
        <div className='space-y-4'>
          ...
          <div>
            <label className='block text-white/70 text-sm mb-2 font-medium'>Closing Time (UTC)</label>
            <input
              type='datetime-local'
              value={data.closing_timestamp}
              onChange={(e) => onChange({ ...data, closing_timestamp: e.target.value })}
              className='w-full px-4 py-3 bg-gray-800 rounded-lg text-white border border-gray-700 focus:border-[#3EB489] focus:outline-none focus:ring-2 focus:ring-[#3EB489]/50 transition-all text-base'
              step='60'
              style={{
                colorScheme: 'dark'
              }}
            ...
            <p className='text-white/50 text-xs mt-2 flex items-center gap-1'>
              ...
              Select date and time in 24-hour format (UTC timezone)
            </p>
          </div>
